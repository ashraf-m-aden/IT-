<template>
  <!-- Start Cloud Hosting Services -->
  <section class="services-area ptb-80 bg-f7fafd">
    <div class="container">
      <div class="row h-100 justify-content-center align-items-center">
        <div class="col-lg-6 col-md-12 services-content">
          <div class="section-title">
            <h2>Systemes et réseaux informatiques</h2>
            <div class="bar" />
            <p>
              Le département Réseaux et Systèmes forme des étudiants capables de
              concevoir, de paramétrer, de sécuriser et d’administrer des
              réseaux de données de type IP utilisant les technologies LAN, WAN
              et WLAN. Il fournit aux entreprises, aux opérateurs et autres, des
              ingénieurs, des techniciens et spécialistes capables de mobiliser
              et d’intégrer des solutions adéquates et des technologies
              appropriées et pertinentes pour résoudre des problèmes relevant
              des domaines des réseaux d’entreprises vers les technologies de
              virtualisation de réseaux et de serveurs et la mutualisation de
              services à travers le Cloud Computing.
            </p>
          </div>

          <div class="row">
            <div class="col-lg-6 col-md-6 col-sm-6">
              <div class="box">
                <feather type="share-2" /> Réseaux de base, les
                fondamentaux
              </div>
            </div>

            <div class="col-lg-6 col-md-6 col-sm-6">
              <div class="box">
                <feather type="globe" /> Réseaux avancées
              </div>
            </div>

            <div class="col-lg-6 col-md-6 col-sm-6">
              <div class="box">
                <feather type="cpu" /> GNS3: maitriser l'émulation des
                résaux
              </div>
            </div>

            <div class="col-lg-6 col-md-6 col-sm-6">
              <div class="box">
                <feather type="shield" /> Securité Informatique
              </div>
            </div>

            <div class="col-lg-6 col-md-6 col-sm-6">
              <div class="box">
                <feather type="layout" /> Installation et configuration
                de windows 7, 8, 10 et 11
              </div>
            </div>

            <div class="col-lg-6 col-md-6 col-sm-6">
              <div class="box">
                <feather type="server" /> Administration Système Windows
                Server (2008, 2012, 2016)
              </div>
            </div>

            <div class="col-lg-6 col-md-6 col-sm-6">
              <div class="box">
                <feather type="layout" /> Administration Système Linux
              </div>
            </div>

            <div class="col-lg-6 col-md-6 col-sm-6">
              <div class="box">
                <feather type="file-text" /> Bureautique (Word, Excel,
                Access, PowerPoint etc.…)
              </div>
            </div>
          </div>
        </div>

        <div class="col-lg-6 col-md-12 services-right-image">
          <img
            v-wow
            src="../../../assets/img/services-right-image/book-self.png"
            class="wow fadeInDown"
            data-wow-delay="0.6s"
            alt="book-self"
          >
          <img
            v-wow
            src="../../../assets/img/services-right-image/box.png"
            class="wow fadeInUp"
            data-wow-delay="0.6s"
            alt="box"
          >
          <img
            v-wow
            src="../../../assets/img/services-right-image/chair.png"
            class="wow fadeInLeft"
            data-wow-delay="0.6s"
            alt="chair"
          >
          <img
            v-wow
            src="../../../assets/img/services-right-image/cloud.png"
            class="wow zoomIn"
            data-wow-delay="0.6s"
            alt="cloud"
          >
          <img
            v-wow
            src="../../../assets/img/services-right-image/cup.png"
            class="wow bounceIn"
            data-wow-delay="0.6s"
            alt="cup"
          >
          <img
            v-wow
            src="../../../assets/img/services-right-image/flower-top.png"
            class="wow fadeInDown"
            data-wow-delay="0.6s"
            alt="flower"
          >
          <img
            v-wow
            src="../../../assets/img/services-right-image/head-phone.png"
            class="wow zoomIn"
            data-wow-delay="0.6s"
            alt="head-phone"
          >
          <img
            v-wow
            src="../../../assets/img/services-right-image/monitor.png"
            class="wow fadeInUp"
            data-wow-delay="0.6s"
            alt="monitor"
          >
          <img
            v-wow
            src="../../../assets/img/services-right-image/mug.png"
            class="wow rotateIn"
            data-wow-delay="0.6s"
            alt="mug"
          >
          <img
            v-wow
            src="../../../assets/img/services-right-image/table.png"
            class="wow fadeInUp"
            data-wow-delay="0.6s"
            alt="table"
          >
          <img
            v-wow
            src="../../../assets/img/services-right-image/tissue.png"
            class="wow zoomIn"
            data-wow-delay="0.6s"
            alt="tissue"
          >
          <img
            v-wow
            src="../../../assets/img/services-right-image/water-bottle.png"
            class="wow zoomIn"
            data-wow-delay="0.6s"
            alt="water-bottle"
          >
          <img
            v-wow
            src="../../../assets/img/services-right-image/wifi.png"
            class="wow fadeInLeft"
            data-wow-delay="0.6s"
            alt="wifi"
          >
          <img
            src="../../../assets/img/services-right-image/cercle-shape.png"
            class="bg-image rotateme"
            alt="shape"
          >

          <img
            v-wow
            src="../../../assets/img/services-right-image/main-pic.png"
            class="wow fadeInUp"
            data-wow-delay="0.6s"
            alt="main-pic"
          >
        </div>
      </div>
    </div>
  </section>
  <!-- End Cloud Hosting Services -->
</template>

<script>
export default {
}
</script>
